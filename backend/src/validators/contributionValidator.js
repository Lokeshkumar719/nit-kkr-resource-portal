...
const validateCreateContribution = (data, file) => {
  ...
  if (type === RESOURCE_TYPES.LECTURES) {
    if (!url || !validator.isURL(url)) {
      throw new ApiError(STATUS_CODES.BAD_REQUEST, 'A valid lecture URL is required.');
    }
...
const validateUpdateContribution = (data) => {
  ...
  if (type === RESOURCE_TYPES.LECTURES || (url !== undefined && url !== '')) {
    if (url && !validator.isURL(url)) {
      throw new ApiError(STATUS_CODES.BAD_REQUEST, 'A valid URL is required.');
    }
  }
};
