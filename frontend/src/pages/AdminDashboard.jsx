...
const ResourcesTab = () => {
  ...
  useEffect(() => {
    if (mode === 'manage_existing' && manageSubjectId) {
      ...
      api
        .get('/resources', { params: { subjectId: manageSubjectId } })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <button
          ...
          className={`flex items-center gap-3 p-4 rounded-xl border transition ${mode === 'add_material' ? 'bg-nit-primary text-white border-nit-primary shadow-md' : 'bg-white text-gray-700 border-slate-300 hover:border-nit-primary hover:shadow-sm'}`}
        >
          <div
            className={`p-2 rounded-lg ${mode === 'add_material' ? 'bg-white/20' : 'bg-blue-50 text-nit-primary'}`}
          >
            <FileText className="w-5 h-5" />
          </div>
          <div className="text-left">
            <p className="font-bold">Add Material</p>
            <p className={`text-xs ${mode === 'add_material' ? 'text-blue-100' : 'text-gray-500'}`}>
        ...
        <button
          ...
          className={`flex items-center gap-3 p-4 rounded-xl border transition ${mode === 'create_subject' ? 'bg-nit-primary text-white border-nit-primary shadow-md' : 'bg-white text-gray-700 border-slate-300 hover:border-nit-primary hover:shadow-sm'}`}
        >
          <div
            className={`p-2 rounded-lg ${mode === 'create_subject' ? 'bg-white/20' : 'bg-blue-50 text-nit-primary'}`}
          >
            <BookOpen className="w-5 h-5" />
          </div>
          <div className="text-left">
            <p className="font-bold">Create Subject</p>
            <p
              className={`text-xs ${mode === 'create_subject' ? 'text-blue-100' : 'text-gray-500'}`}
            >
              Add a new course
            </p>
          </div>
        ...
        <button
          ...
          className={`flex items-center gap-3 p-4 rounded-xl border transition ${mode === 'manage_existing' ? 'bg-nit-primary text-white border-nit-primary shadow-md' : 'bg-white text-gray-700 border-slate-300 hover:border-nit-primary hover:shadow-sm'}`}
        >
          <div
            className={`p-2 rounded-lg ${mode === 'manage_existing' ? 'bg-white/20' : 'bg-blue-50 text-nit-primary'}`}
          >
            <LayoutDashboard className="w-5 h-5" />
          </div>
          <div className="text-left">
            <p className="font-bold">Manage Existing</p>
            <p
              className={`text-xs ${mode === 'manage_existing' ? 'text-blue-100' : 'text-gray-500'}`}
            >
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-nit-primary text-white py-2.5 rounded-lg hover:bg-blue-900 transition flex justify-center items-center gap-2"
            >
              {loading ? (
      ...
      {mode === 'manage_existing' && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300 space-y-4">
          {manageMsg.text && (
            <div
              className={`p-3 rounded-md text-sm ${manageMsg.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}
            >
              {manageMsg.text}
            </div>
          )}
                  ...
                  {(resourceFilter === 'ALL'
                    ...
                    if (editingResource === r._id) {
                      return (
                        <div
                          key={r._id}
                          className="p-4 rounded-lg border border-nit-primary bg-blue-50/50 space-y-3"
                        >
                          <div className="grid sm:grid-cols-2 gap-3">
                            <input
                              type="text"
                              className="w-full p-2 border border-gray-300 rounded-md text-sm outline-none focus:ring-2 focus:ring-nit-primary"
                          ...
                          <div className="flex gap-2 justify-end">
                            <button
                              ...
                              className="px-3 py-1.5 text-sm rounded-md bg-gray-200 text-gray-700 hover:bg-gray-300 transition"
                            >
                              Cancel
                            </button>
                            <button
                              onClick={() => handleSaveEdit(r._id)}
                              disabled={savingEditId === r._id}
                              className="px-3 py-1.5 text-sm rounded-md bg-nit-primary text-white hover:bg-blue-900 transition flex items-center gap-2"
                            >
                              {savingEditId === r._id ? (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div
                            className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${typeColors[r.type] || 'bg-gray-100 text-gray-600'}`}
                          >
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              {r.fileName && (
                                <span className="text-xs text-gray-400">• {r.fileName}</span>
                              )}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {r.fileKey && (
                            <button
                              onClick={async () => {
                                try {
                                  const res = await getResourceDownloadUrl(r._id);
                              ...
                              className="p-2 rounded-lg bg-nit-primary text-white hover:bg-blue-900 transition"
                              title="Download"
                            >
                          ...
                          {confirmDeleteId === r._id ? (
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="px-2 py-1.5 rounded-lg bg-red-600 text-white text-xs font-semibold hover:bg-red-700 transition disabled:opacity-50"
                              >
                                {deletingId === r._id ? (
                              ...
                              <button
                                ...
                                className="px-2 py-1.5 rounded-lg bg-gray-200 text-gray-600 text-xs font-semibold hover:bg-gray-300 transition"
                              >
                                No
                              </button>
                            </div>
...
const SeniorsTab = () => {
  ...
  useEffect(() => {
    if (mode === 'manage_existing' && manageBranch) {
      setLoadingManage(true);
      api
        .get('/mentors', { params: { branch: manageBranch } })
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      ...
      if (editFormData.company?.trim()) {
        payload.experiences = [
          { company: editFormData.company.trim(), role: 'Employee', type: 'Placement' },
        ];
      } else {
        payload.experiences = [];
      }
      ...
      if (editFormData.achievements?.trim()) {
        payload.achievements = editFormData.achievements
          .split(',')
          .map((a) => a.trim())
          .filter((a) => a);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <button
          onClick={() => setMode('add_senior')}
          className={`flex items-center gap-3 p-4 rounded-xl border transition ${mode === 'add_senior' ? 'bg-nit-primary text-white border-nit-primary shadow-md' : 'bg-white text-gray-700 border-slate-300 hover:border-nit-primary hover:shadow-sm'}`}
        >
          <div
            className={`p-2 rounded-lg ${mode === 'add_senior' ? 'bg-white/20' : 'bg-blue-50 text-nit-primary'}`}
          >
            <Users className="w-5 h-5" />
          </div>
          <div className="text-left">
            <p className="font-bold">Add Profile</p>
            <p className={`text-xs ${mode === 'add_senior' ? 'text-blue-100' : 'text-gray-500'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                className="w-full p-2.5 border border-gray-300 rounded-lg"
                placeholder="e.g. Codeforces Expert, GSOC'22"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-nit-primary text-white py-2.5 rounded-lg font-medium hover:bg-blue-900 transition flex justify-center items-center h-[46px] mt-6 disabled:opacity-70 gap-2"
            >
          ...
          {manageBranch && (
            <div className="border-t border-slate-200 pt-4 mt-2">
              ...
              {loadingManage ? (
                ...
                <div className="text-center py-8 text-gray-400 text-sm">
                  No profiles found for this branch.
                </div>
              ) : (
                <div className="space-y-2">
                  {(manageYearFilter === 'ALL'
                    ? manageMentors
                    : manageMentors.filter((m) => m.currentYear === manageYearFilter)
                  ).map((m) => {
                    if (editingId === m._id) {
                      return (
                        <div
                        ...
                        >
                            ...
                            {editFormData.year === 'Alumni' && (
                              <>
                                <input
                                  ...
                                  className="w-full p-2 border border-gray-300 rounded-md text-sm outline-none focus:ring-2 focus:ring-nit-primary"
                                  value={editFormData.batchStart}
                                  onChange={(e) =>
                                    setEditFormData({ ...editFormData, batchStart: e.target.value })
                                  }
                                />
                                <input
                                  type="number"
                                  ...
                                  className="w-full p-2 border border-gray-300 rounded-md text-sm outline-none focus:ring-2 focus:ring-nit-primary"
                                  value={editFormData.batchEnd}
                                  onChange={(e) =>
                                    setEditFormData({ ...editFormData, batchEnd: e.target.value })
                                  }
                                />
                              </>
                            )}
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (isSelected)
                                      setEditFormData({
                                        ...editFormData,
                                        tags: editFormData.tags.filter((tag) => tag !== t),
                                      });
                                    else
                                      setEditFormData({
                                        ...editFormData,
                                        tags: [...editFormData.tags, t],
                                      });
                                  }}
                          ...
                          <div className="flex gap-2 justify-end mt-3 border-t border-blue-200 pt-3">
                            ...
                            <button
                              ...
                              disabled={savingEditId === m._id}
                              className="px-4 py-2 text-sm rounded-md bg-nit-primary text-white hover:bg-blue-900 transition flex items-center gap-2 font-medium"
                            >
                              {savingEditId === m._id ? (
                                <Loader className="w-4 h-4 animate-spin" />
                              ) : (
      ...
      {loading && !contributions.length ? (
        ...
        <div className="bg-white p-12 rounded-xl shadow-sm border border-slate-300 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-gray-500 text-sm max-w-sm">
            There are no pending contributions to review at this time.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {contributions.map((item) => (
            <div
              key={item._id}
              className="bg-white p-5 rounded-xl shadow-sm border border-slate-300 flex flex-col md:flex-row justify-between items-start gap-4 transition-all hover:border-slate-400"
            >
              {editingId === item._id ? (
                <div className="flex-1 min-w-0 space-y-3 w-full">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      ...
                      disabled={processing.id === item._id}
                      className="px-3 py-1.5 bg-nit-primary text-white text-sm font-medium rounded-lg hover:bg-blue-900 transition flex items-center gap-2 disabled:opacity-50"
                    >
                      {processing.id === item._id && processing.action === 'edit' ? (
                    ...
                    <button
                      onClick={() => setEditingId(null)}
                      disabled={processing.id === item._id}
                      className="px-3 py-1.5 bg-slate-100 text-slate-600 text-sm font-medium rounded-lg hover:bg-slate-200 transition disabled:opacity-50"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span
                        className={`px-2 py-0.5 text-xs font-semibold rounded uppercase ${TYPE_COLORS[item.type] || 'bg-gray-100 text-gray-700'}`}
                      >
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button
                      ...
                      title="Approve"
                    >
                      {processing.id === item._id && processing.action === 'approve' ? (
                    ...
                    <button
                      ...
                      title="Reject"
                    >
                      {processing.id === item._id && processing.action === 'reject' ? (
                        <Loader className="animate-spin w-5 h-5" />
                      ) : (
                        <X size={18} />
                      )}
