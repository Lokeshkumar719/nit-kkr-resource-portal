...
export default function Contribute() {
  ...
  const handleSubmitBug = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        toast.error('Bug reporting backend endpoint is not currently available.');
      } else {
  ...
  const handleSubmitResource = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        toast.error('Resource contribution backend endpoint is not currently available.');
      } else {
        toast.error(err.response?.data?.message || 'Unable to submit your contribution. Please try again.');
      }
    } finally {
      setLoading(false);
    }
